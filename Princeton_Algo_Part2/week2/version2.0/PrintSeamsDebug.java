...
public class PrintSeamsDebug {
    ...
    public static void main(String[] args) {
        ...
        StdOut.printf("height = %d  width = %d \n", carver.height(), carver.width());
        StdOut.printf("Horizontal seam: { ");
        int[] horizontalSeam2 = carver.findHorizontalSeam();
        for (int x : horizontalSeam2)
            StdOut.print(x + " ");
        StdOut.println("}");
        printSeam(carver, horizontalSeam2, HORIZONTAL);

        carver.removeHorizontalSeam(horizontalSeam2);
        StdOut.printf("height = %d  width = %d \n", carver.height(), carver.width());
        StdOut.printf("Horizontal seam: { ");
        int[] horizontalSeam3 = carver.findHorizontalSeam();
        for (int x : horizontalSeam3)
            StdOut.print(x + " ");
        StdOut.println("}");
        printSeam(carver, horizontalSeam3, HORIZONTAL);
    }
