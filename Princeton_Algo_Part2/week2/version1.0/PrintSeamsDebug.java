...
public class PrintSeamsDebug {
    ...
    public static void main(String[] args) {
        ...
        StdOut.printf("height = %d  width = %d \n", carver.height(), carver.width());
        StdOut.printf("Vertical seam: { ");
        int[] verticalSeam2 = carver.findVerticalSeam();
        for (int x : verticalSeam2)
            StdOut.print(x + " ");
        StdOut.println("}");
        printSeam(carver, verticalSeam2, VERTICAL);

        carver.removeVerticalSeam(verticalSeam2);
        StdOut.printf("height = %d  width = %d \n", carver.height(), carver.width());
        StdOut.printf("Vertical seam: { ");
        int[] verticalSeam3 = carver.findVerticalSeam();
        for (int x : verticalSeam3)
            StdOut.print(x + " ");
        StdOut.println("}");
        printSeam(carver, verticalSeam3, VERTICAL);
    }
